...
class BfsTxSpiderSpider(scrapy.Spider):
    ...
    def __init__(self, **kwargs):
        ...
        self.out_path = kwargs.get('out', './data/BFS')  # 输出文件夹路径
        ...
        # 保证爬虫终止条件只有一个
        conds = [('epa', self.epa), ('depth', self.depth)]
    ...
    def satisfy_ending_cond(self, seed: str, **kwargs):
        ...
        if self.cond_name == 'epa':
            if cond_value >= self.cond_value:
                return True
            return False
        elif self.cond_name == 'depth':
            if kwargs[self.cond_name] >= self.cond_value:
                return True
            return False
